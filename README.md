# CampusGuide 校园导游咨询系统

CampusGuide 是一个基于 C++17、Qt 6 Quick/QML 和 CMake 实现的校园导游咨询系统。系统把校园地图抽象为无向带权图，用节点表示景点、路口和道路转折点，用边表示可通行道路，从而支持景点查询、最短路径规划、附近设施搜索和可视化路网编辑。
...
- **景点查询**：在地图上点击景点标记，或从查询弹窗中按类型、关键词筛选景点，查看名称、类型、简介和位置。
- **地图定位**：选择景点后，地图视图会跳转到对应位置，并高亮当前目标。
- **最短路径**：选择起点和终点后，后端使用 Dijkstra 算法计算路线，前端在地图上绘制路径。
- **可选路径**：路径弹窗可展示多条较短的简单路径，便于比较不同路线。
- **附近搜索**：选择当前位置和设施类型后，系统返回距离最近的若干设施，并可直接查看到达路线。

### 编辑功能

- **路网可视化**：编辑模式下显示景点节点、普通路点和道路边，便于检查校园路网。
- **节点编辑**：支持新增、拖拽、修改和删除节点；景点节点可同步编辑名称、类型和简介。
- **道路编辑**：支持连接两个节点形成道路，也支持删除已有道路。
...
```text
Spot  景点业务信息：nodeId、name、type、intro
Node  路网节点：id、x、y
Edge  道路边：to、weight
```

`Spot` 不保存独立景点 ID，也不直接保存坐标。景点的唯一标识就是 `nodeId`，并通过这个 `nodeId` 绑定到同 ID 的 `Node`。这样景点显示、路径计算和拖拽编辑都使用同一份坐标数据，避免重复存储导致不同步。
...
- `nodeId`：景点绑定的路网节点 ID，也是景点的唯一标识。
- `name`：景点名称。
- `type`：景点类型。
- `intro`：景点简介。

当前代码只读取 `nodeId,name,type,intro` 格式，不再兼容历史格式。景点坐标统一由 `nodes.txt` 中相同 ID 的节点保存。
...
景点节点和普通路点都存放在这里。节点是否为景点，通过是否存在 `Spot.nodeId == Node.id` 判断。项目约定新增景点节点使用 `1-999` 的空闲 ID，新增普通路点使用 `1000` 及以上 ID。
...
文件只保存道路连接关系。程序加载时会根据节点坐标自动计算边权并添加双向边，因此路网是无向带权图。
...
| 景点索引 | `CampusGraph` | 使用 `spotNodeToIndex` 按 `nodeId` 快速查找景点 |
| 最小堆 | `MinHeap` | 为 Dijkstra 提供最小距离节点弹出能力 |
| 最短路径 | `PathFinder::shortestPath` | 计算两点之间的最短路线 |
| 可选路径 | `PathFinder::allSimplePaths` | 基于最短路径偏离生成多条较短简单路径 |
